	  ...
	  if(i != j && i != k && j != k && i != l && j != l && k != l)
	    {
	      ...
	      chemin[4] = l;
	      int dist = distance[0][i] + distance[i][j] + distance[j][k] + distance[k][l];
	      tsp_seq_optimized (5, dist, chemin,  1 | (1<<i) | (1<<j) | (1<<k) | (1<<l)) ;
         }
}

void tsp_ompcol3_opt()
{
  int i,j,k;
#pragma omp parallel for collapse(3) schedule(runtime)
 for (i=1; i < nbVilles; i++)
       ...
       if(i != j && i != k && j != k)
         {
          ...
          chemin[3] = k;
          int dist = distance[0][i] + distance[i][j] + distance[j][k];
          tsp_seq_optimized (4, dist, chemin,  1 | (1<<i) | (1<<j) | (1<<k)) ;
         }
}

void tsp_ompcol2_opt()
{
  int i,j;
#pragma omp parallel for collapse(2) schedule(runtime)
 for (i=1; i < nbVilles; i++)
     ...
     if(i != j)
         {
          ...
          chemin[2] = j;
          int dist = distance[0][i] + distance[i][j];
          tsp_seq_optimized (3, dist, chemin,  1 | (1<<i) | (1<<j) );
         }
}
